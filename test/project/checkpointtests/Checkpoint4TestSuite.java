...
public class Checkpoint4TestSuite {
    ...
    public void testFileWritten() throws Exception {
        ...
        Assertions.assertTrue(allLines.size() >= 3, 
                "Make sure that the output is all written to 3 comma-separated lines");
        String[] results = allLines.get(0).split(",");
        Assertions.assertTrue(results.length >= 0, "Make sure exactly one sequence is outputted for each input");
    }
}
